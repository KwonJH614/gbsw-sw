...
public class DashboardService {
    ...
    private StatsDto getStats(Long userId) {
        Map<String, Object> row = jdbc.queryForMap(
            "SELECT " +
            "  COUNT(CASE WHEN e.completed = false THEN 1 END) AS activeCourses, " +
            "  COUNT(CASE WHEN e.completed = true  THEN 1 END) AS completedCourses, " +
            "  COALESCE(SUM(p.watched_seconds) / 60, 0) AS totalWatchedMinutes, " +
    ...
    private List<RecentCourseDto> getRecentCourses(Long userId) {
        return jdbc.query(
            ...
            "       p.lesson_id AS lastLessonId, p.watched_seconds AS watchedSeconds " +
            "FROM progress p " +
    ...
    public List<ActivityDto> getActivities(Long userId, int limit) {
        return jdbc.query(
            ...
            "  JOIN enrollments e ON e.id = p.enrollment_id WHERE e.user_id = ? " +
            "  UNION ALL " +
            "  SELECT 'COMPLETED', CONCAT(l.title, ' 완료'), p.updated_at " +
            "  FROM progress p JOIN lessons l ON l.id = p.lesson_id " +
            "  JOIN enrollments e ON e.id = p.enrollment_id WHERE e.user_id = ? AND p.completed = true " +
            "  UNION ALL " +
